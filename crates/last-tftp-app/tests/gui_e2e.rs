...
async fn mock_server_rrq(content: Vec<u8>, blksize: u16) -> (u16, tokio::task::JoinHandle<()>) {
    ...
    let handle = tokio::spawn(async move {
        ...
        // 等 client ACK(OACK)
        let (_n, from) = match recv_with_timeout(&sock, &mut buf).await {
            Ok(r) => r,
            Err(e) => {
                eprintln!("[mock server] {e}");
                return;
...
fn gui_get_end_to_end() {
    ...
    rt.block_on(async {
        ...
        let (server_port, server_handle) = mock_server_rrq(payload.clone(), 1468).await;

        let mut app = App::headless_new();
        app.remote_host = "127.0.0.1".into();
        app.remote_port = server_port;
        app.remote_file = "data.bin".into();
        app.local_file_str = std::env::temp_dir().join("last_tftp_gui_e2e_get.bin").display().to_string();
        app.blksize = 1468;
        app.window = 1;
...
fn gui_put_end_to_end() {
    ...
    rt.block_on(async {
        ...
        let src = std::env::temp_dir().join("last_tftp_gui_e2e_put_src.bin");
